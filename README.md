...
- **Onboarding.** New servers get a welcome message with a **✨ Start a design session** button (a two-field form: what are we building, and an optional site to borrow the feel from) and a role picker; `/help` repeats it. A server's first three sessions carry quick tips on the kickoff message, and every kickoff has the role picker. Running out of renders posts an upgrade button in the thread instead of a dead end. Idle Free sessions are deleted after `RETENTION_DAYS`.
- **Never crash-loops on configuration.** Missing or malformed variables, a bad token, or the Message Content intent turned off keep the bot offline but the web server up: `/health` passes and `/` shows a setup page listing each problem and its fix. Pasted `# comments` in variables are stripped and flagged. On Railway, the public URL and data directory come from Railway's domain and volume automatically.
